...
public class MinimalSteps {
    ...
    public static void main(String[] args) {
        ...
        int res = new MinimalSteps().minimalSteps(maze);
    ...
    public int minimalSteps(String[] maze) {
        ...
        for (int i = 0; i < nb; i++) {
            ...
            dd[i] = d;
            // 从某个点到终点不需要拿石头
            // 没有nb+1那个机关，所有此时代表，该机关是最后一个触发的阶段，然后将直接走向终点，那此时所需要的步数是多少。
            dist[i][nb + 1] = d[tx][ty];
        }
        ...
        for (int i = 0; i < nb; i++) {
            ...
            for (int k = 0; k < ns; k++) {
                ...
                if (dd[i][midX][midY] != -1 && startDist[midX][midY] != -1) { // 机关i可以到石头（midX,midY），并且起点也可以抵达此石头
                    if (tmp == -1 || tmp > dd[i][midX][midY] + startDist[midX][midY]) {
                        tmp = dd[i][midX][midY] + startDist[midX][midY];
                    }
                }
            }
            // 经过上一轮循环，得出tmp: 机关I到石头+石头到起点的最短路径

            // dist中 dist[i][nb+1] 代表机关i是最后一个机关，并且到终点的距离是dist[i][nb+1]  dist[i][nb],代表机关i是第一个被连接的机关，并且从 起点->石头->机关i   的距离是dist[i][nb]
            // 其他0<=j<=nb-1 dist[i][j]代表机关i到机关j经过某一个石头o所经过的最短路径
            dist[i][nb] = tmp;

            //下面两个for循环就是计算任意机关i->j 经过石头k,所最短的距离dist[i][j]
        ...
        // 无法达成的情形
        for (int i = 0; i < nb; i++) {
            // 这多么机关，只要有一个机关满足以下条件就无法达成情形的原因是：满足-1的唯一条件是机关i的四周都是# ，那么机关i必定无法被触发。
